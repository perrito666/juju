...
func (e *manualEnviron) SetConfig(cfg *config.Config) error {
	...
	if e.storage == nil {
		...
		if envConfig.useSSHStorage() {
			...
			storageTmpdir := path.Join(paths.DefaultDataDir, storageTmpSubdir)
...
`
	script = fmt.Sprintf(
		...
		mongo.ServiceName(""),
		utils.ShQuote(paths.DefaultDataDir),
		utils.ShQuote(paths.DefaultLogDir),
	)
...
func (e *manualEnviron) StorageDir() string {
	return path.Join(paths.DefaultDataDir, storageSubdir)
}
